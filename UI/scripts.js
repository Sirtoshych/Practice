var articles = [
  {
   ...
   },
   {
    ...
    summary: 'В декрет о тунеядцах готовятся изменения: уже в ближайшее время список социальных иждивенцев могут серьезно сократить. Об этом TUT.BY рассказал один из лидеров кампании «Говори правду» Андрей Дмитриев после встречи с вице-спикером парламента Болеславом Пирштуком',
    createdAt: new Date('2017-02-27T22:00:00'),
    author: 'Максим Стефаненко',
   ...
   },
   {
    ...
    author: 'Ре',
    content: 'По его словам, коллегия должна состояться в марте. Российской стороне было направлено предложение провести ее до 10 марта, но оно пока осталось без ответа. На заседании планируется обсудит'
   ...
   },
   {
    ...
    summary: 'Минтранс не будет вносить изменения в механизм сбора дорожного налога, при этом ведомство уделит внимание ремонту местных дорог. Об этом на пресс-конференции сказал министр транспорта и коммуникаций Беларуси Анатолий Сивак.',
...
var tags = ['Politics','Sport','Society','Finance','Health','Technology','People','Cars','Football','Popular']
function getArticles(skipNumber, amount, filterConfig){
	skipNumber = skipNumber || 0;
	amount = amount || articles.length;
	var sorted = articles;
	if (filterConfig != 0){
		if (filterConfig.author != 0){
			sorted = sorted.filter(function (item) {
                return item.author == filterConfig.author
            })
		}
		if (filterConfig.createdAt != 0){
			sorted = sorted.filter(function (item){
